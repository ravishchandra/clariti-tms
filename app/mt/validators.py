...
def validate_string(
    ...
    if src_placeholders != tgt_placeholders:
        errors.append(f"Placeholder mismatch: source has {src_placeholders}, translated has {tgt_placeholders}")

    # 2. ICU shape check
    if key_icu_shape == "plural":
        if "{" not in translated or "plural," not in translated:
            errors.append('ICU plural shape required: translated string must contain "{" and "plural,"')

    # 3. Structural tag placeholders
    if key_has_structural_tags:
        for placeholder in tag_map:
            if placeholder not in translated:
                errors.append(f"Structural tag placeholder missing in translation: {placeholder!r}")

    # 4. Max length
    if max_length is not None and len(translated) > max_length:
        errors.append(f"Translation exceeds max length: {len(translated)} > {max_length}")
    ...
    for term in glossary_terms:
        if term.get("do_not_translate") and term["source_term"] not in translated:
            errors.append(f"Brand term must appear verbatim: {term['source_term']!r} missing from translation")
