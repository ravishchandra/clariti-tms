...
- [x] ~~Register the marketing domain or change `site.url`.~~ Temporarily set to `https://clariti-tms.vercel.app` so canonicals, sitemap, JSON-LD, and OG match the live URL. **Permanent fix still needed**: register a real domain (e.g. clariti.dev / clariti-tms.dev / claritihq.com) and bump `site.url` again, then add the custom domain in Vercel project settings → Domains.
- [x] ~~**Layout JSON-LD `Organization.logo`** references `${site.url}/logo.svg` — file does not exist in `marketing/public/`. Add `logo.svg`, `favicon.ico`, `apple-touch-icon.png`, and `icon.png` so the JSON-LD validates and OG/search results render properly.~~ Done 2026-05-24 — adopted the dashboard's PNG logo as the shared brand mark. `marketing/public/logo-512.png` ships; JSON-LD now points there. `icon.png` + `apple-icon.png` live under `marketing/src/app/` (Next.js convention).
...
- [x] ~~**No favicon / app icon / Apple touch icon.** Only the inline `<Logo>` SVG exists; browser tabs show the default Next.js icon. Add `marketing/src/app/icon.tsx` + `apple-icon.tsx` (Next.js convention).~~ Done 2026-05-24 — `marketing/src/app/icon.png` (96×96) and `apple-icon.png` (686×686) ship as PNGs (dashboard brand mark), picked up automatically by Next.js. Visible `<Logo>` component now renders the same PNG.
