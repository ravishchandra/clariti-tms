...
export function Nav() {
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--color-line)]/60 bg-[var(--color-ink-0)]/70 backdrop-blur-md">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-6">
        <Link href="/" className="group">
          <Logo />
        </Link>

        <nav className="hidden items-center gap-7 text-[13px] text-[var(--color-text-soft)] md:flex">
          {site.navLinks.map((l) => (
            <Link
              key={l.href}
              href={l.href}
        ...
        <div className="flex items-center gap-2">
          <a
            ...
            aria-label="Star ClaritiTMS on GitHub (opens the repo in a new tab — GitHub requires you to be signed in to actually star)"
            className="hidden text-[13px] text-[var(--color-text-soft)] hover:text-[var(--color-text)] md:inline-flex md:items-center md:gap-1.5"
          >
